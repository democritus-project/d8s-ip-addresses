...
@pytest.mark.network
@pytest.mark.xfail(
    reason="pre-existing, unrelated to tooling migration: live network/HTTP request to external service", strict=False
)
def test_ip_whois_1():
...
@pytest.mark.xfail(
    reason="pre-existing, unrelated to tooling migration: asserts on live IANA registry data that has drifted",
    strict=False,
)
